...
def create_reserva_atomic(conn, id_sala, fecha, id_turno, participantes_ci):
    ...
    try:
        cur.execute('SELECT COUNT(*) AS cnt FROM reserva WHERE id_sala=%s AND fecha=%s AND id_turno=%s FOR UPDATE', (id_sala, fecha, id_turno))
        if cur.fetchone()['cnt'] > 0:
            raise Exception('La sala ya está reservada en ese turno')
        ...
        if capacidad is None:
            raise Exception('La sala no tiene capacidad definida')
        if tipo_sala is None:
            raise Exception('La sala no tiene tipo definido')
        if len(participantes_ci) > capacidad:
            raise Exception('Excede capacidad de la sala')

        for ci in participantes_ci:
            cur.execute("""
                SELECT 
                    SUM(ppa.rol='docente') AS is_docente, 
                    SUM(pa.tipo='posgrado') AS is_pos 
            ...
            is_pos = False
            if r:
                is_doc = (r.get('is_docente') or 0) > 0
                is_pos = (r.get('is_pos') or 0) > 0

            if tipo_sala == 'docente' and not is_doc:
                raise Exception(f'Participante {ci} no puede reservar sala docente')
            if tipo_sala == 'posgrado' and not (is_doc or is_pos):
            ...
            if not is_doc and not is_pos:
                ...
                if cur.fetchone()['cnt'] >= 2:
                    raise Exception(f'Participante {ci} excede límite diario')

                cur.execute("""
                    SELECT COUNT(*) AS cnt 
                    FROM reserva_participante rp 
                    JOIN reserva r ON rp.id_reserva = r.id_reserva 
                    WHERE rp.ci_participante=%s AND YEARWEEK(r.fecha,1)=YEARWEEK(%s,1) AND r.estado='activa'
                """, (ci, fecha))
                if cur.fetchone()['cnt'] >= 3:
                    raise Exception(f'Participante {ci} excede límite semanal')
