...
def derive_role_from_participante(email):
    ...
    try:
        ...
        if not row:
            return 'alumno'
        ci = row['ci']
        cur.execute(\"\"\"SELECT COUNT(*) cnt FROM participante_programa_academico ppa WHERE ppa.ci_participante = %s AND ppa.rol = 'docente'\"\"\", (ci,))
        if cur.fetchone()['cnt'] > 0:
            return 'docente'
        cur.execute(\"\"\"SELECT COUNT(*) cnt FROM participante_programa_academico ppa JOIN programa_academico pa ON ppa.id_programa = pa.id_programa WHERE ppa.ci_participante = %s AND pa.tipo = 'posgrado'\"\"\", (ci,))
        if cur.fetchone()['cnt'] > 0:
            return 'posgrado'
        return 'alumno'
    finally:
...
def register():
    ...
    if not (ci and nombre and apellido and email and password):
    ...
    def decorator(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            try:
                ...
                return jsonify({'error':'token invÃ¡lido', 'detail': str(e)}), 401
